...
class CausalInterventionGenerator:
    ...
    @torch.no_grad()
    def generate_from_prompt(self, prompt, num_effects=4, outdir=None,**gen_kwargs):
        import json
...
def find_project_root(start: str) -> str:
    """
    Walk upwards until we find a folder that contains data/test_prompts.txt
    """
    ...
    for _ in range(15):
        if (p / "data" / "test_prompts.txt").exists():
            return str(p)
        p = p.parent
    raise RuntimeError("Could not find project root (missing data/test_prompts.txt)")


def run_test_prompts_batch(
    adapter_path=adapter_path,
    num_effects=10,
    max_prompts=None
):
    """
    Runs your ORIGINAL generate_from_prompt() for each prompt in data/test_prompts.txt
    and saves outputs to:
      src/experiments/1/causal_dreamer/samples/<row_id>/
    """
    # -------- project root + paths --------
    project_root = find_project_root(__file__)
    project_root = Path(project_root)

    prompt_file = project_root / "data" / "test_prompts.txt"
    out_root = project_root / "src" / "experiments" / "1" / "outputs"
    samples_dir = out_root / "causal_dreamer2"
    samples_dir.mkdir(parents=True, exist_ok=True)

    print("PROJECT_ROOT:", project_root)
    print("PROMPT_FILE:", prompt_file)
    print("OUT_DIR:", samples_dir)
    ...
    if not prompts:
        raise ValueError("test_prompts.txt is empty")

    if max_prompts is not None:
        prompts = prompts[:max_prompts]

    # -------- use your original generator (UNCHANGED) --------
    generator = CausalInterventionGenerator(adapter_path=adapter_path)

    # -------- loop over prompts --------
    for row_id, prompt in enumerate(prompts):
        print(f"\n[{row_id+1}/{len(prompts)}] Prompt:", prompt)
        out_dir = samples_dir / f"{row_id:06d}"
        save_dir = (samples_dir / f"{row_id:06d}")
        save_dir.mkdir(parents=True, exist_ok=True)

        images = generator.generate_from_prompt(
            prompt=prompt,
            num_effects=num_effects,
            outdir=str(save_dir)
        )

        # save images into our experiment folder
        image_list = []
        rows_meta = []

        for i, (effect, img) in enumerate(images):
            safe_effect = effect.replace(" ", "_").replace("/", "_")
            img_path = out_dir / f"{i:02d}_{safe_effect[:60]}.png"
            img.save(img_path)
            image_list.append(img)
            rows_meta.append({
                "index": i,
        ...
        grid_path = None
        if image_list:
            cols = 5 if len(image_list) >= 5 else len(image_list)
            rows = (len(image_list) + cols - 1) // cols
            grid = create_image_grid(image_list, rows=rows, cols=cols)
        ...
        meta = {
            "row_id": row_id,
            "prompt": prompt,
            "num_effects": len(images),
            "rows": rows_meta,
            "grid_path": str(grid_path) if grid_path else None,
            "method": "causal_dreamer_original_generate_from_prompt",
    ...
    print("\n✅ Finished all prompts. Output:", samples_dir)

if __name__ == "__main__":
    # test_generator()
    #test_with_debug()
    adapter_path = "checkpoint/causal_adapter_epoch_200.pt"
    run_test_prompts_batch(adapter_path=adapter_path, num_effects=8, max_prompts=None)
